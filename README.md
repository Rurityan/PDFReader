...
- OCR 截取会进入顺序后台队列，可连续框选多个区域；结果需用户确认后保存，并显示识别中、待确认、红色“待挂载（启动时清理）”或绿色“待挂载（独立保留）”状态。
- “OCR 待处理记录”区支持挂载、生成音频、仅保存和右键按指定模型生成或删除。仅保存可将未挂载 OCR 标记为独立保留；清除独立保留项只会将其从待处理区隐藏。
- 顶部“资源”可管理当前 PDF 的 OCR 与音频资源，显示 OCR/VOC 数量统计，支持 OCR 标题搜索和已挂载/未挂载筛选；未挂载 OCR 可选择具体书签挂载。音频始终依附于 OCR，删除 OCR 会联动删除音频和截图。
...
HTML5 阅读包为目录结构，包含 `index.html`、`app.js`、`app.css`、`manifest.json`、`manifest.js`、按页懒加载的 `pages/*.webp` 和关联 `audio/`。网页支持 25%-300% 百分比缩放、页面横向/纵向滚动、在阅读区顶部或底部继续滚轮触发上一页/下一页、OCR 标题/正文搜索定位、显示/隐藏当前页 OCR、本页朗读和从当前页起的连续朗读。页面上下均预留 `25vh`，翻页时按相同的视口留白定位，页面内容保持水平居中。OCR 标题显示在区域外，并通过半透明连接线指向选区。移动端使用默认折叠、宽度约为屏幕 `68%` 的左侧书签抽屉，书签内容支持横向滚动；父书签点击后保持抽屉打开以便继续浏览子书签，叶子书签或点击抽屉外部会自动收起。工具栏支持横向触摸滚动，页面支持横向和纵向触摸滚动，首次加载页面会按视口宽度自动适配，之后可用双指手势自由缩放。书签树默认折叠，浏览器 `localStorage` 会按阅读包保存上次页面和书签展开状态。音频按需请求，播放器显示加载中、缓冲中、播放中、暂停和失败状态，并提供当前音频的可拖动进度条和时间显示；连续朗读时仅预加载下一页已有音频，停止或结束后释放预加载对象。`manifest.js` 是为直接双击 `index.html` 准备的内嵌清单；因此不依赖本地服务器也可以打开，`manifest.json` 仍保留供调试和二次处理使用。
