...
HTML5 阅读包为目录结构，包含 `index.html`、`app.js`、`app.css`、`manifest.json`、`manifest.js`、按页懒加载的 `pages/*.webp` 和关联 `audio/`。网页支持 25%-300% 百分比缩放、页面横向/纵向滚动、在阅读区顶部或底部继续滚轮触发上一页/下一页、OCR 标题/正文搜索定位、显示/隐藏当前页 OCR、本页朗读和从当前页起的连续朗读。页面上下均预留 `25vh`，翻页时按相同的视口留白定位，页面内容保持水平居中。OCR 标题显示在区域外，并通过半透明连接线指向选区。书签树默认折叠，浏览器 `localStorage` 会按阅读包保存上次页面和书签展开状态。音频按需请求，播放器显示加载中、缓冲中、播放中、暂停和失败状态，并提供当前音频的可拖动进度条和时间显示；连续朗读时仅预加载下一页已有音频，停止或结束后释放预加载对象。`manifest.js` 是为直接双击 `index.html` 准备的内嵌清单；因此不依赖本地服务器也可以打开，`manifest.json` 仍保留供调试和二次处理使用。
