...
顶部“资源”窗口以当前 PDF 的 OCR 记录为管理单位，支持按 OCR 标题搜索，并按已挂载/未挂载状态筛选。每个条目展示页码、挂载状态和音频可用状态；无可用音频的条目提供“生成音频”按钮，点击后从当前配置的 Voice Model 列表中选择模型生成。可以删除其全部音频，或删除 OCR 及关联的音频、截图。不会提供独立音频对象的创建、导入或保留入口，`TtsAudioRecords.OcrRecordId` 外键和启动清理共同保证音频必须关联 OCR。
...
3. 在页面上框选区域，应用将选区渲染为图片后立即加入 OCR 历史队列；后台单消费者按顺序调用 `Scripts/ocr_worker.py`，因此用户可在识别期间继续截取，不会并发启动多个 OCR worker。
4. 队列项依次处于“识别中...”、“待确认”或“待挂载”状态。识别中项不可保存、挂载或清除。
5. 用户可修改当前队列项的标题和正文，点击“仅保存”后才创建数据库记录；仅保存不会自动挂载书签，记录转为“待挂载”。
6. 保存时若同页存在书签则自动挂载；否则记录以“待挂载”保留在历史区。挂载按钮为金色，未选中书签时会创建 OCR 所在页书签；选中不同页书签时必须在“跨页挂载”与“创建书签”之间明确选择。
7. OCR 记录可以在书签树中拖拽到另一个书签下重新挂载。
8. 用户可以为 OCR 记录生成 TTS 音频，音频文件写入配置的 voice 目录并建立数据库记录。
9. 应用退出时，尚未挂载到书签的 OCR 待处理记录会被直接丢弃；其截图、音频等关联资源也会清理。截图本身不作为受保护数据处理。
