...
- 提供页码输入跳转和低倍率页面缩略图预览，并缓存缩略图以减少重复渲染。
...
数据库和配置路径固定为程序目录下的 `user_data`。截图和音频目录可以在设置窗口中修改。页面缩略图缓存固定使用 `user_data/cache`，按 PDF 文档 UUID 分目录保存；PDF 文件大小、最后修改时间、页数或缩放参数变化时会自动失效并重建。删除或重新绑定 PDF 时会清理对应缩略图缓存。
...
- 书签树中的 OCR 项带有特殊标记：没有音频时显示 `OCR`，存在可用音频时显示绿色 `VOC`。正文通过右键菜单查看，播放和生成音频也通过右键菜单完成。
...
4. 测试一次截取、持续截取、取消截取和右键取消。
5. 确认 OCR 结果后检查 `OcrRecords` 是否产生记录，取消确认时不应产生记录。
6. 创建多级书签，测试拖拽层级、OCR 重新挂载、重命名、名称点击跳转、删除和撤回。
7. 配置多个 Voice Model，测试名称下拉选择、键值对增删，以及实际请求使用 `voice_id`。
8. 为已保存 OCR 生成音频，检查 `TtsAudioRecords`、`OCR/VOC` 标记和顶部“朗读/停止播放”状态。
9. 创建各类标注，确认页面立即显示、未点击保存前 PDF 文件不变，点击保存后可以重新读取。
10. 关闭应用时测试未保存标注和未挂载 OCR 的处理提示。
