...
- 提供页码输入跳转、可编辑百分比缩放、`Ctrl + 滚轮` 缩放、虚拟化连续滚动阅读和低倍率页面缩略图预览，并缓存缩略图以减少重复渲染。
...
HTTP TTS 音频响度归一化默认关闭。启用后必须在设置中指定有效的 `ffmpeg.exe`，TTS 响应会先保存为临时文件，再通过 `loudnorm=I=-16:TP=-1.5:LRA=11` 重编码为 MP3；只有归一化成功才会写入音频记录。归一化失败会删除临时文件并将生成操作视为失败。该处理只作用于本应用通过 HTTP TTS 新生成的音频，不处理 REST 导入或已有音频文件。
...
应用启动后会在后台运行一次一致性清理：删除无效文档或书签关联的 OCR、无主音频记录及其已知资源；失效的书签父级关系会被提升为根书签。REST 导入的未挂载 OCR 通过外部导入标记保留；用户点击“仅保存”后的未挂载 OCR 通过独立保留标记保留。其他手动未挂载 OCR 会被清理。待处理区的隐藏状态只影响主界面队列展示，不影响资源管理、OCR 正文、音频或清理豁免。该任务不扫描资源目录删除未被数据库引用的任意文件。
...
- OCR 默认隐藏，显示/隐藏按钮通过 DOM `hidden` 属性立即切换当前页覆盖层；显示时标题位于 OCR 框外，以半透明短线连接到区域。有有效音频的 OCR 框显示播放按钮，并可参加“朗读本页”；没有音频的 OCR 仍显示框和正文，不阻止导出，也不显示不可用的播放按钮。连续朗读期间“连续朗读”按钮禁用；用户手动翻页不会中断连续音频队列，因此仍可暂停、继续或停止。
- 导出包支持 25%-300% 的百分比缩放，页面容器按缩放后的实际尺寸提供横向和纵向滚动；阅读容器使用显式网格居中，页面左右居中，上下均预留 25vh 空间。翻页落点根据页面容器的实际顶部和底部位置计算，使上下落点使用同一个视口留白，不再按总滚动长度比例估算。阅读区在顶部继续向上滚轮或底部继续向下滚轮累计达到阈值后翻页，页面内部滚动不会误触发翻页。OCR 坐标与页面图片按同一比例重新计算。使用浏览器原生 `<audio preload="none">`，支持单条播放、本页顺序播放、从当前页起跨页连续朗读、暂停/继续/停止和页码跳转；播放器显示加载中、缓冲中、播放中、暂停、完成或失败状态，并提供当前音频的可拖动进度条和时间显示。连续朗读时额外使用隐藏的 `Audio` 对象预加载下一页已有音频，最多只保留下一页，停止或结束后释放。
...
- HTML5 书签树默认全部折叠；每个有子书签的节点使用原生 `<details>`，展开/折叠状态和上次阅读页通过浏览器 `localStorage` 按阅读包保存。无法使用本地存储时回退到默认折叠和第 1 页，不影响阅读。
- HTML5 移动端使用单列阅读布局和默认折叠、宽度约为屏幕 68% 的左侧书签抽屉；书签栏独立支持横向滚动，父书签点击后保持抽屉打开，叶子书签和抽屉外点击会自动收起。工具栏支持横向触摸滚动，阅读区支持页面横向/纵向触摸滚动。首次加载页面会按阅读区宽度自动适配，用户可通过双指手势自由缩放；手动缩放后不再被窗口尺寸变化覆盖。

顶部“资源”窗口以当前 PDF 的 OCR 记录为管理单位，显示 OCR 总数和拥有可用音频的 VOC 记录数，支持按 OCR 标题搜索，并按已挂载/未挂载状态筛选。每个条目展示页码、挂载状态和音频可用状态；无可用音频的条目提供“生成音频”按钮，点击后从当前配置的 Voice Model 列表中选择模型生成。未挂载项可选择当前 PDF 的具体书签进行挂载。可以删除其全部音频，或删除 OCR 及关联的音频、截图。不会提供独立音频对象的创建、导入或保留入口，`TtsAudioRecords.OcrRecordId` 外键和启动清理共同保证音频必须关联 OCR。
...
| `IsExternalImport` | 是否由本地 REST 导入且尚未挂载 |
| `AllowStandalone` | 是否允许未挂载 OCR 独立保留，不参与启动清理 |
| `IsHiddenFromProcessingQueue` | 是否从主界面 OCR 待处理记录隐藏 |
| `CreatedAtUtc` | 创建时间 |
...
3. 在页面上框选区域，应用将选区渲染为图片后立即加入 OCR 待处理队列；后台单消费者按顺序调用 `Scripts/ocr_worker.py`，因此用户可在识别期间继续截取，不会并发启动多个 OCR worker。
4. 队列项依次处于“识别中...”、“待确认”、红色“待挂载（启动时清理）”或绿色“待挂载（独立保留）”状态。识别中项不可保存、挂载或清除。
5. 用户可修改当前队列项的标题和正文。OCR 选区的“保存”遵循同页书签自动挂载规则；“仅保存”不自动挂载，并为记录写入独立保留标记。
6. 未挂载且未受保护的手动记录会在启动清理时删除；独立保留和 REST 导入的记录不会。清除独立保留项只会持久化隐藏其待处理显示，OCR、音频与资源管理条目不受影响。
7. 挂载按钮为金色，未选中书签时会创建 OCR 所在页书签；选中不同页书签时必须在“跨页挂载”与“创建书签”之间明确选择。资源管理也可将未挂载 OCR 选择性挂载到具体书签。
8. OCR 记录可以在书签树中拖拽到另一个书签下重新挂载；待处理项右键菜单支持删除和按指定 Voice Model 生成音频。
9. 用户可以为 OCR 记录生成 TTS 音频，音频文件写入配置的 voice 目录并建立数据库记录。
...
- 右键菜单支持跳转到所在页面、修改名称、按名称筛选选择父书签并挂载为子书签、脱离父书签和删除。
- “查找当前页书签”与按名称查找都会逐级展开父书签、选中目标并滚动书签树到对应位置；名称查找使用不区分大小写的字符串包含匹配。
- 点击书签树空白处会取消当前书签选择，避免旧书签影响新 OCR 的挂载目标。
- 删除含子书签的节点前弹出确认窗口。
