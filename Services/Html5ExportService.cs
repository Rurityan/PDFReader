...
const loadManifest=window.PDFREADER_MANIFEST?Promise.resolve(window.PDFREADER_MANIFEST):fetch('manifest.json').then(r=>r.json());loadManifest.then(m=>{state.manifest=m;state.storedView=readViewState();document.title=String(m.title||'PDFReader').replace(/\.pdf$/i,'')+' - H5';$('title').textContent=m.title;$('page-count').textContent=m.pageCount;$('page').max=m.pageCount;installSearch();buildBookmarks();go(state.storedView.page||1)}).catch(e=>{document.body.innerHTML='<p style="padding:24px;color:#b42318">无法加载阅读包清单：'+e.message+'</p>'});
...
function go(n,preserve=false){if(!state.manifest)return;const previousPage=state.page;const targetPage=Math.min(pageCount(),Math.max(1,Number(n)||1));if(!preserve&&!state.continuous)stop();state.page=targetPage;state.storedView.page=state.page;writeViewState();$('page').value=state.page;const image=$('page-image');const direction=targetPage<previousPage?-1:1;image.onload=()=>{applyLayout();renderOcr();requestAnimationFrame(()=>setPagePosition(direction))};image.src=path(state.page)}
...
function updateContinuousReadButton(){$('read-all').disabled=state.continuous}
